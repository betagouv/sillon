...
Nous préconisons d'utiliser une pipeline de [_CI/CD_](https://en.wikipedia.org/wiki/CI/CD) décrite dans votre repository. Si votre repository est par exemple sur GitHub nous ne voyons pas l'utilité d'utiliser un outil tiers comme [_CircleCI_](https://circleci.com/) plutôt que [_GitHub Actions_](https://github.com/features/actions) (comme ça au moins vous avez tout dans un endroit).
...
4.  Je fais mes tests unitaires avec [_Jest_](https://github.com/jestjs/jest) ;
5.  Je “build” mon projet ;
6.  Je fais mes tests d'intégration end-to-end avec [_Cypress_](https://github.com/cypress-io/cypress) _(on ne les recommande pas, ils sont très lourds à maintenir pour un gain limité comparé à [utiliser Storybook pour décrire son applicatif](#_3fcnbnmh2siy))_ ;
...
- Si la RAM sature, là c'est le worker qui crash car… “[_Out of memory_](https://en.wikipedia.org/wiki/Out_of_memory)”, il faut relancer la pipeline ;
...
_Si vous disposez d'une messagerie interne comme Mattermost ou Slack nous vous conseillons de brancher la pipeline de votre projet sur un channel type “monprojet-monitoring” afin d'être sûr de ne rien manquer de la vie technique du projet._
