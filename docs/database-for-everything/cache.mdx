...
2.  Si c'est de la [latence](<https://fr.wikipedia.org/wiki/Latence_(informatique)>) sur le réseau, assurez-vous que l'applicatif et la base de données soient dans la même zone géographique (même [datacenter](https://fr.wikipedia.org/wiki/Centre_de_donn%C3%A9es)) ;
3.  Si c'est la base de données, ajustez aussi sa gamme (CPU/RAM) voire passez sur un stockage [SSD](https://fr.wikipedia.org/wiki/SSD) plutôt que [HDD](https://en.wikipedia.org/wiki/Hard_disk_drive).
