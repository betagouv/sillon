...
Sachez qu'une PostgreSQL est capable de gérer au sein d'une colonne des fichiers faisant maximum 1 Go chacun, il faut pour cela utiliser le type _BYTEA_ (qui utilise le [TOAST](https://www.postgresql.org/docs/current/storage-toast.html)).
...
Un élément important qui est mentionné dans [le chapitre légal](#le-légal) et qui doit être pris en compte dans votre calcul : **la CNIL recommande de ne pas conserver les données utilisateurs plus de 2 ans**.
...
Notez que **les technologies comme tus ou S3 ne gèrent pas les règles de sécurité propres à votre métier** (qui peut uploader ? qui peut lire ce fichier ?). C'est à vous de gérer cette partie. Nous abordons cela plus en détail dans [le chapitre sur la sécurité](#sécuriser-les-urls-des-fichiers-hébergés).
