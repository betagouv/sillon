...
Sachez qu'une PostgreSQL est capable de gérer au sein d'une colonne des fichiers faisant maximum 1 Go chacun, il faut pour cela utiliser le type _BYTEA_ (qui utilise le [_TOAST_](https://www.postgresql.org/docs/current/storage-toast.html)).

_Il existe aussi la possibilité de stocker des “[large objects](https://www.postgresql.org/docs/current/largeobjects.html)” qui peuvent faire jusqu'à 4 To chacun, il faut une étape supplémentaire pour les ajouter avant de les référencer dans une table traditionnelle. Et cela pose des questions de volumétrie qui seront abordés plus loin._

**Vous devez spécifier et quantifier ce que vous allez héberger pour dire oui ou non aux buckets S3.**

Un élément important qui est mentionné dans [_le chapitre légal_](#le-légal) et qui doit être pris en compte dans votre calcul : **la CNIL recommande de ne pas conserver les données utilisateurs plus de 2 ans**.
...
- En utilisant la PostgreSQL pour le stockage **il ne faut jamais essayer de produire vous-même la logique d'upload d'un fichier**. Cela implique beaucoup de technique : headers HTTP, le “resumable upload” (fait de reprendre une upload si la connexion internet est perdue durant quelques secondes)... **Nous préconisons plutôt d'utiliser [_tus_](https://github.com/tus)** ([_client_](https://github.com/tus/tus-js-client) et [_serveur_](https://github.com/tus/tus-node-server)) sachant qu'ils proposent [_une intégration dans l'API Next.js_](https://github.com/tus/tus-node-server/tree/main/packages/server#example-integrate-tus-into-nextjs) ;
...
3.  Le frontend peut uploader son fichier directement sur l'URL signée (en utilisant une librairie cliente compatible comme [_tus_](https://github.com/tus/tus-js-client)) et obtient l'URL finale du fichier uploadé ;
...
Notez que **les technologies comme tus ou S3 ne gèrent pas les règles de sécurité propres à votre métier** (qui peut uploader ? qui peut lire ce fichier ?). C'est à vous de gérer cette partie. Nous abordons cela plus en détail dans [_le chapitre sur la sécurité_](#sécuriser-les-urls-des-fichiers-hébergés).

_Écrire et lire des fichiers dans votre base de données va forcément utiliser un peu plus de ressources (notamment la bande passante). N'hésitez pas à “augmenter” les performances de votre base si besoin._
