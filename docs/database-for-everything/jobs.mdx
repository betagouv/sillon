...
Une “[_job queue_](https://en.wikipedia.org/wiki/Job_queue)” est utile :
...
2.  Si jamais votre partenaire change de format de données dans son payload, on sait d'avance que cela ne passerait pas de votre côté [_car vous validez la donnée en entrée_](#validation-des-données-entrantes), il vaut mieux avoir le payload complet utilisé pour investiguer et savoir ce qu'il faut ajuster.

Nous préconisons d'utiliser la librairie [_pg-boss_](https://github.com/timgit/pg-boss) en lieu de place d'outils dédiés comme “Redis Pub/Sub”, Kafka (...). Elle va savoir reproduire toute la logique des “job queues” en ayant quelques tables dans votre base de données. De plus :
...
- Quand vous ferez un [_backup_](https://en.wikipedia.org/wiki/Backup) ou un “restore” de votre base de données, les événements seront synchronisés avec l'état global des données de votre produit.
...
- À vous de choisir votre stratégie de rejeu en fonction de la criticité du job :
  ...
  - Vous pouvez vous reposer sur [_Sentry_](#faire-la-vigie-en-étant-outillé) pour être notifié d'un job qui est abandonné afin que vous puissiez investiguer et tenter de le rejouer manuellement.
