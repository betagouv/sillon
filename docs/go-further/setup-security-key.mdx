...
Copier la partie `DevSrvsID:876876876` (cela peut être `/dev/hidraw*` sous linux ou `DevSrvsID:******` sous max).
...
Si pour le faire dans un

```
$ git config commit.gpgsign true
```

C'est pour la signature des commits avec votre clé SSH !

### Installer et configurer GnuPG (version alternative)

Si vous n'avez pas choisi la signature de commit avec votre clé SSG, vous pouvez le faire avec OpenPGP.
