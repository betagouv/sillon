...
Pour faire connexion avec [_le chapitre sur les coûts liés au temps_](#réfléchir-aux-coûts-liés-au-temps), il faut garder en tête la gradation suivante qui définit aussi le temps nécessaire pour chaque étape d'un produit (du plus court au plus long) :
