...
Que nous utilisions [un hébergement de fichiers en base ou via des buckets S3](#stockage-de-fichiers-au-revoir-buckets-s3), il faut garantir que les URLs de nos fichiers sensibles soient éphémères. Même si vous avez un identifiant de fichier aléatoire (UUID) et que vous estimez qu'on ne peut pas deviner l'URL :
