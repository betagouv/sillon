...
:::note Quoi acheter ?
La marque la plus connue et très appréciée historiquement est [Yubico](https://www.yubico.com/) (suédois) avec ses Yubikeys, mais dorénavant il existe plein d'alternatives (que nous n'avons pas testés).
:::
...
Comme indiqué plus haut les Yubikey sont des clefs USB/dongles utilisés principalement pour sécuriser l'authentification des utilisateurs à leur compte, mais pas seulement.
Elles peuvent aussi servir à authentifier les commits git.

Pour la suite de cette documentation Le type de Yubikey (USB-C, USB-A, etc.) n'est pas important. En revanche, nous travaillerons avec la série 5.

## **Installation**

La Yubikey vient avec plusieurs applications fournies par Yubico.
Celle qui nous intéresse dans notre cas est le Yubikey-manager.
...
La suite de cette documentation est inspirée de celle fournie par Yubico : [https://developers.yubico.com/SSH/Securing_SSH_with_FIDO2.html](Securing SSH Authentication with FIDO2)
...
```bash
ssh-keygen -t ed25519-sk -O resident -O application=ssh:github -C "clé pour accès à Github"
  ```
...
Ensuite, on retrouvera dans le répertoire courant les fichiers publiques et privés des clefs :
- id_ed25519_sk_rk_github
- id_ed25519_sk_rk_github.pub
Ces fichiers peuvent être effacés, car nous les récupèrerons à partir de la yubikey dans une étape suivante.

Dans le cas où on doit générer une autre clef SSH, on lui spécifiera un autre nom.
Dans la commande suivante, elle aura pour ID "ssh:nubo"
```bash
ssh-keygen -t ed25519-sk -O resident -O application=ssh:nubo -C "clé pour accès au cloud Nubo"
  ```
...
e929dde6...    ssh:nubo    openssh            openssh
  ```
...
Documentation de référence : [https://github.com/YubicoLabs/sign-git-commits-yubikey](https://github.com/YubicoLabs/sign-git-commits-yubikey)
...
### **Importer sa clé pgp dans Github**
...
![https://media1.tenor.com/m/O1rWEILXLFIAAAAC/yatta-hiro-heroes.gif](https://media1.tenor.com/m/O1rWEILXLFIAAAAC/yatta-hiro-heroes.gif)
