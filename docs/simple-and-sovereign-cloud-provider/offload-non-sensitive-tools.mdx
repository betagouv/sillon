...
Les hébergeurs applicatifs vont vous faciliter la vie pour voir les logs de votre application, gérer les ressources… Pourtant il y a certains outils annexes qui sont très demandés comme [_Sentry pour du monitoring d'erreurs_](#faire-la-vigie-en-étant-outillé). Cet outil propose une offre SaaS mais celle-ci dépend d'un hébergeur étranger. Vous pourriez tenter de l'héberger vous-même mais ça revient à complexifier votre stack (surtout qu'il faut que votre hébergeur accepte une image Docker/Buildpack sinon cela risque d'être compliqué).
...
2.  Soit vous pouvez vous adresser à une entreprise qui fait du pseudo-SaaS de cet outil comme [_Empreinte Digitale_](https://empreintedigitale.fr/) (mais il existe sûrement d'autres acteurs) ;
